...
namespace Mathtone.Sdk.Data.Tests {
	public abstract class DbDataReaderExtensionsTest<CN, RDR> : IDbDataReaderExtensionsTest<CN, RDR>
		...
		//[Fact]
		//public async Task ConsumeAsync_() => Assert.Equal(
		//	new[] { "A", "B", "C" },
		//	await Connect().Used(cn => cn
		//		.CreateCommand<DbCommand>(Queries.TestQuery)
		//		.ExecuteReader()
		//		.ConsumeAsync(r => Task.FromResult(r["value"].ToString()))
		//		.ToArrayAsync()
		//	)
		//);

		//[Fact]
		//public async Task ConsumeAsync_3() => Assert.Equal(
		//	new[] { "A", "B", "C" },
		//	await Connect().Used(cn => cn
		//		.CreateCommand<DbCommand>(Queries.TestQuery)
		//		.ExecuteReader()
	...
	//		var cmd = CreateCommand("THIS IS $DEFINITELY$ A TEST")
	//		740	.WithTemplate("$DEFINITELY$", "PROB'ABLY", escape);
